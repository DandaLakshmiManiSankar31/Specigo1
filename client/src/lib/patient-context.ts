...
export function formatMedicalHistoryForAI(history: PatientMedicalHistory | null | undefined): string {
  ...
  if (history.dailyCalorieTarget) dietInfo.push(`Daily calorie target: ${history.dailyCalorieTarget} kcal`);
  ...
  if (history.medicalDietType?.length && !history.medicalDietType.includes("None")) {
    dietInfo.push(`Medical goal: ${history.medicalDietType.join(', ')}${history.medicalDietTypeOther ? ` (${history.medicalDietTypeOther})` : ''}`);
  }
  if (history.dietFollowFrequency) dietInfo.push(`Diet adherence: ${history.dietFollowFrequency}`);
  if (history.dietRevisionDays) dietInfo.push(`Meal variety preference: ${history.dietRevisionDays}`);
