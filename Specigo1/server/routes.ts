  ...
  app.post(api.ai.chat.path, async (req, res) => {
    try {
      ...
      try {
        ...
        if (chatMode === "report-analysis") {
          ...
          if (isNewSession || isFirstMessage) {
            const electrolyteCriticalReminder = reportContext
              ...
              : "";
            enrichedMessage = `${context ? `Patient context: ${context}\n\n` : ""}${reportContext ? `Lab Report:\n${reportContext}` : ""}${electrolyteCriticalReminder}\n\nPatient request: ${message}\n\nIMPORTANT INSTRUCTIONS FOR THIS PLAN:\n1. Carefully read the patient's medical history, conditions, medications, allergies, fitness goal, diet type, and cuisine preference provided above — and tailor the plan accordingly.\n2. For EACH meal section (Breakfast, Mid-Morning Snack, Lunch, Evening Snack, Dinner), include on the FIRST line after the section header: **~XXX kcal | Protein Xg | Carbs Yg | Fat Zg** (total for that meal).\n3. Then list 2-4 food items with portions and individual calorie counts.\n4. On the LAST line of your response only, add the overall macro tag: [MACROS: Protein X%, Carbs Y%, Fat Z%].`;
          } else {
        ...
        const maxTokens = chatMode === "report-analysis" ? (isFirstMessage ? 400 : 300) : chatMode === "symptom-followup" ? 1200 : chatMode === "diet-planner" ? 1200 : symptomaticConcluding ? 350 : 100;
