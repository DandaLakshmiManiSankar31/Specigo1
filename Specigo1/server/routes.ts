      ...
      try {
        ...
        if (chatMode === "report-analysis") {
          ...
          if (isNewSession || isFirstMessage) {
            ...
            enrichedMessage = `${context ? `Patient context: ${context}\n\n` : ""}${reportContext ? `Lab Report:\n${reportContext}` : ""}${electrolyteCriticalReminder}${indianCuisineInstruction}\n\nPatient request: ${message}\n\n*** CRITICAL — DO NOT ASK ANY QUESTIONS. Do NOT ask what the patient eats, their preferences, their routine, or any clarifying information. You ALREADY have everything you need in the patient context above. IMMEDIATELY generate a complete, structured diet plan. ***\n\nINSTRUCTIONS FOR THIS PLAN:\n1. Use ONLY the patient context provided above — it already contains their medical history (conditions, medications, allergies, surgeries, family history), diet history (meals per day, food type, cuisine preference, fitness goal, diet adherence, revision interval), and all profile details. No additional information is needed.\n2. Cross-reference the patient's medical conditions (e.g., diabetes, hypertension, kidney disease, thyroid, PCOD, anaemia) with their diet history and adjust food choices, portion sizes, and macro ratios accordingly.\n3. If the patient has allergies mentioned in their history, strictly EXCLUDE those foods from all meal suggestions.\n4. Respect the patient's stated food type (vegetarian / non-vegetarian / vegan / eggetarian) from their diet history at all times.\n5. For EACH meal section (Breakfast, Mid-Morning Snack, Lunch, Evening Snack, Dinner), include on the FIRST line after the section header: **~XXX kcal | Protein Xg | Carbs Yg | Fat Zg** (total for that meal).\n6. Then list 2-4 food items with portions and individual calorie counts.\n7. On the LAST line of your response only, add the overall macro tag: [MACROS: Protein X%, Carbs Y%, Fat Z%].\n8. DO NOT end with any questions. DO NOT ask for more details. Just provide the complete plan.`;
          } else {
