        ...
        {expandedSections.diet && (
          <CardContent className="p-5 space-y-4 bg-white dark:bg-slate-800">
            ...
            <FormField label="When do you have these meals? (Timings)">
              <Input
                placeholder="e.g. 8 AM, 1 PM, 4 PM, 8 PM"
                value={formData.mealTimings || ""}
                onChange={(e) => updateField("mealTimings", e.target.value)}
                data-testid="input-meal-timings"
              />
            </FormField>
            ...
            <FormField label="Type of Diet to Follow (select all that apply)">
            ...
            <FormField label="How often do you follow your diet plan?">
              <Select value={formData.dietFollowFrequency || ""} onValueChange={(v) => updateField("dietFollowFrequency", v)}>
                <SelectTrigger data-testid="select-diet-follow-frequency"><SelectValue placeholder="Select frequency" /></SelectTrigger>
                <SelectContent>
                  {["Always (strictly follow every day)", "Most of the time (5–6 days/week)", "Sometimes (3–4 days/week)", "Occasionally (1–2 days/week)", "Rarely", "Not following any plan currently"].map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}
                </SelectContent>
              </Select>
            </FormField>

            <FormField label="After how many days do you want your diet plan to be revised?">
              <Select value={formData.dietRevisionDays || ""} onValueChange={(v) => updateField("dietRevisionDays", v)}>
                <SelectTrigger data-testid="select-diet-revision"><SelectValue placeholder="Select revision interval" /></SelectTrigger>
                <SelectContent>
                  {["7 days (Weekly)", "14 days (Bi-weekly)", "21 days", "30 days (Monthly)", "45 days", "60 days (Bi-monthly)", "90 days (Quarterly)", "Only when needed"].map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}
                </SelectContent>
              </Select>
            </FormField>
          </CardContent>
